...
- LLM providers: `ollama`, `gemini`/Vertex AI, and vLLM via OpenAI-Compatible (`api_base`); `gemini-2.5-flash` is the default Miro vision model — avoid `llava:7b` (timeout/poor captions); Ollama `gemma4:e2b-mlx` crashes on images, while vLLM `gemma4-e2b` still needs Text & Image / VISION flow for Captioning. Index Settings Contextual Retrieval lists visible models; Captioning requires `supports_image_input` / VISION flow.
