...
- UI rebrand (Jira AI-72): main app matches `ticket-analyzer-agent` with Claude-style minimal chrome; login/auth must follow superplay.co marketing energy (vibrant magenta hero, playful gaming aesthetic) — not corporate/dark split-screen layouts.
- SuperPlay brand assets are `web/public/logo.svg` (wordmark lockup) and `web/public/favicon.svg` (icon); use these across web Opal logo/icon components.
...
- Homelab EE feature testing uses `ENABLE_PAID_ENTERPRISE_EDITION_FEATURES=true` and `LICENSE_ENFORCEMENT_ENABLED=false` on api_server, celery workers, and web; `/admin/theme` custom logo upload accepts PNG/JPEG only (export `logo.svg` first).
...
- `web/lib/shared` design tokens are shared with mobile (`file:` dep); web-only SuperPlay theming uses unlayered CSS overrides in `web/src/app/css/` — do not edit `web/lib/shared/tokens/*.json` for web-only rebrand.
- Compose builds `api_server`, `background`, and `web_server` from local source; `bin/dev` / `mise run dev` stacks dev + GPU compose overlays with Compose watch.
