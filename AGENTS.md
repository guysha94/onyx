...
- Prefer manual DNS-01 Let's Encrypt for homelab (certbot prints TXT record for Route 53; no AWS API creds); use HTTP-01 only if port 80 is forwarded and manual DNS is inconvenient; manual certs are not autorenewed — re-run `mise run dev:gpu:init-certs` before expiry.
...
- Prefer OpenAI-Compatible (not a dedicated vLLM provider tile) for self-hosted vLLM used as Contextual Retrieval and Captioning LLMs; models need visibility plus a Text & Image / vision override to appear in those Index Settings pickers.
...
- Docker Compose config lives at `deployment/docker_compose/.env`; env vars apply at container creation — run `docker compose up -d --force-recreate` on affected services after changes. Local start: `mise dev`; GPU/HTTPS: `mise run dev:gpu` merges `docker-compose.dev-ssl.yml` (nginx 80/443 + Let's Encrypt; `.env.nginx` with `CERTBOT_CHALLENGE=manual-dns`; bootstrap via `dev:gpu:init-certs`). Backend images install from hashed `backend/requirements/default.txt` (not `pyproject.toml` directly) — after adding deps run `uv export` per `backend/requirements/README.md` before rebuild; `dev:gpu` does not hot-reload Python into `api_server`/`background` without compose watch — rebuild/recreate those services after backend code or dependency changes.
- Docker Compose config lives at `deployment/docker_compose/.env`; env vars apply at container creation — run `docker compose up -d --force-recreate api_server web_server` after changes. Local start: `mise dev`; GPU/HTTPS: `mise run dev:gpu` merges `docker-compose.dev-ssl.yml` (nginx 80/443 + Let's Encrypt; `.env.nginx` with `CERTBOT_CHALLENGE=manual-dns`; bootstrap via `dev:gpu:init-certs`). Homelab EE testing uses `ENABLE_PAID_ENTERPRISE_EDITION_FEATURES=true` and `LICENSE_ENFORCEMENT_ENABLED=false` on api_server, celery workers, and web.
- Primary Atlassian/Jira instance is `superplaystudio.atlassian.net`.
- Custom connector priorities: Monday.com (AI-69; uses `monday` SDK via `MondayApiClient` API `2025-10`; workspace-first traversal; every item doc carries searchable `workspace_id`/`workspace_name`/`board_id`/`board_name` metadata plus section-text hierarchy), Miro (AI-70; visual assets only; doc IDs `miro__boardId__itemId`; user-typed `item.data.title` as `asset_title`/`semantic_identifier`), and Mixpanel.
- Custom source forks register only in `backend/onyx/connectors/fork_registry.py` (never edit `registry.py` body), tag shared-file edits with `# FORK: <name>`, log in `FORK_CHANGES.md`, and use static API-token auth with `LoadConnector`/`PollConnector` for MVP.
...
- LLM providers: `ollama`, `gemini`/Vertex AI, and vLLM via OpenAI-Compatible (`api_base`); `gemini-2.5-flash` is the default Miro vision model — avoid `llava:7b` (timeout/poor captions) and `gemma4:e2b-mlx` (Ollama crash on images). Index Settings Contextual Retrieval lists visible models; Captioning also requires vision (`supports_image_input` / VISION flow).
...
- Agent Knowledge (`SourceHierarchyBrowser` → `/api/hierarchy-nodes`) only lists pre-indexed `HierarchyNode`s (not live vendor APIs); Drive emits folder nodes; GitHub/Monday do not emit hierarchy nodes so repos/workspaces stay empty; Confluence/Jira implement hierarchy but nodes must actually be indexed.
