"""Delete all Onyx GitHub connector-credential pairs.

For each GitHub cc_pair:
...
--wait to block until GitHub cc_pairs disappear from indexing status.

Usage:
  export ONYX_PAT="onyx_pat_..."
  uv run delete_github_connectors.py --dry-run
  uv run delete_github_connectors.py
  uv run delete_github_connectors.py --wait
...
def select_github_connectors(
    ...
    for connector_status in connector_statuses:
        ...
        if (connector.get("source") or "").lower() != GITHUB_SOURCE:
            continue
...
def wait_for_github_deletions(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if not remaining:
            print("All targeted GitHub cc_pairs are gone.")
            return

        print(f"Waiting for {len(remaining)} GitHub cc_pair(s) to finish deleting...")
    ...
    raise TimeoutError(
        f"Timed out after {timeout_seconds}s waiting for GitHub cc_pair deletion"
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Delete all GitHub Onyx connector-credential pairs via admin API"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print GitHub connectors that would be deleted without calling Onyx",
    )
    parser.add_argument(
        "--wait",
        action="store_true",
        help="Block until targeted GitHub cc_pairs disappear from indexing status",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    with httpx.Client(headers=headers, timeout=120) as client:
        ...
        to_delete = select_github_connectors(
            connector_statuses,
        ...
        github_total = sum(
            1
            for connector_status in connector_statuses
            if (connector_status.get("connector") or {}).get("source") == GITHUB_SOURCE
        )
        deleting = github_total - len(to_delete)

        print(f"GitHub cc_pairs total: {github_total}")
        if deleting:
