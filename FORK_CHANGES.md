...
**Perm-sync resilience: private-board skip + fail-closed** (see `plans/miro_perm-sync_resilience_f76c4c89.plan.md`):

| File                                                              | Edit                                                                                                                                                                                                                                                                                                                             | Status                         |
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------ |
| `backend/onyx/connectors/miro/connector.py`                       | `BoardAccessForbiddenError` (fork-local exception); `_extract_miro_error_code` helper; `_get_json` now discriminates 403 `code 6.0108` (per-board access forbidden → `BoardAccessForbiddenError`) from missing-scope 403 (systemic → `InsufficientPermissionsError`); `_process_board` and `retrieve_all_slim_docs_perm_sync` catch `BoardAccessForbiddenError` per board and skip with a warning so neither loop aborts on a private board. ACL-model invariant (org-wide → Onyx `public` is safe while every Onyx user is a Miro-org member) documented in the perm-sync section comment. | net-new file, no marker needed |
| `backend/tests/unit/onyx/connectors/miro/test_miro_perm_sync.py` | New tests: perm-sync skips 6.0108 board; perm-sync stays fatal on missing scope; indexing skips private board; `_get_json` 403 discrimination                                                                                                                                                                                    | net-new file                   |
